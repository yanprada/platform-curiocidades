...
app = DjangoDash('Receitas_pct',external_stylesheets=[dbc.themes.CYBORG],add_bootstrap_links=True)

#*************MODIFICAR O path_download PARA A PASTA DE DOWNLOAD DO SEU PC*************
cwd = os.getcwd()
path_download = Path(cwd,"core/Bases/Prefeitura/Receitas/receita_ano_pct.csv")
    ...
    dbc.Row(
            [
                dbc.Col(dcc.Graph(id='graph-with-slider'),
                        width=12, xs={'size': 12,  "offset": 0}
                        ),
            ]
        )
    ,
    html.P('Escolha o ano:'),
    dcc.Slider(
        id='time-slider',
        min=df['Data'].min(),
        max=df['Data'].max(),
